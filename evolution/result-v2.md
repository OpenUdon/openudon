...
The public Ramen boundary is accepted.

## Accepted Direction

- Ramen is the public UWS authoring, review, package, and executor-handoff tool.
...
- Ramen owns product-level lifecycle APIs even when they look reusable: iCoT, prompt transcripts,
...
1. Move low-risk review/package helpers into Ramen and keep the handoff wire shape stable.
2. Move iCoT lifecycle and JSON completion fallback into Ramen while keeping OpenAPI operation
...
Ramen now has local `internal/authoring` helpers for progressive iCoT lifecycle, draft/transcript
