...
func writeRuntimeDataFile(result Result, intent *rollout.Intent, _ projectPolicy) error {
	...
	for _, block := range file.Body().Blocks() {
		if (block.Type() == "inputs" || block.Type() == "credentials") && len(block.Labels()) == 0 {
	...
	return atomicfile.Write(result.DataPath, data, 0o644)
}
