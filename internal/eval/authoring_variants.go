...
func normalizeAuthoringVariants(variants *AuthoringVariants) error {
	...
	for i := range variants.Variants {
		...
		variant.ExpectedTopIssueCode = strings.TrimSpace(variant.ExpectedTopIssueCode)
		variant.ExpectedTopIssueSlot = strings.TrimSpace(variant.ExpectedTopIssueSlot)
		...
		if variant.ExpectedTopIssueCode != "" {
			family := icotreport.FailureFamilyForReadiness(variant.ExpectedTopIssueCode)
			if family == icotreport.FailureUnknown {
				return fmt.Errorf("authoring variant %q has unsupported expected_top_issue_code %q", variant.ID, variant.ExpectedTopIssueCode)
			}
			if variant.ExpectedFailureFamily != "" && family != variant.ExpectedFailureFamily {
				return fmt.Errorf("authoring variant %q expected_top_issue_code %q maps to %q, not expected_failure_family %q", variant.ID, variant.ExpectedTopIssueCode, family, variant.ExpectedFailureFamily)
			}
		}
