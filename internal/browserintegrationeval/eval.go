	...
	return []gate{
		{
			...
			RequiredPasses: []string{
				...
				"TestLiveAuthorRecordsOnlyHumanSelectedCompatibleMFAKind",
				"TestLiveOutputReviewIsBoundedTypedAndSafe",
				"TestLiveOutputReviewIndependentlyRejectsUnsafeProofShapes",
				"TestAuthenticatedAuthoringV2ReconstructsTOTPAndScalarOutputs",
		...
		},
		{
			...
			RequiredPasses: []string{
				...
				"TestHumanInputCompletionRequiresCompatibleHumanReviewedChallengeKind",
				"TestHumanInputCheckpointIsADistinctProtocolState",
				"TestCompletionAuthorsReviewedOutputsWithActionTimeProofs",
				"TestCompletionAcceptsZeroAndSixteenOutputsButRejectsSeventeen",
				"TestCompletionRejectsUnsafeStaleAmbiguousAndDuplicateOutputs",
				"TestProtocolV1IsRejectedWithoutLaunchingBrowser",
				...
				"TestBuildAuthorsEveryReviewedMFAKindExactly",
				"TestBuildSelectsOldestSufficientTypedOutputProfile",
				"TestBuildSortsResolvedOutputSelectionsAndRejectsUnsafeProofs",
		...
		},
		{
			...
			Args:       []string{"go", "test", "-v", "./schemas", "./uws1", "./browserauthentication", "-run", "Test(ValidateBrowser|CanonicalBrowser|BrowserSourceProfile|BrowserProfileSchema|BrowserAuthentication|Browser17|Validate_UWS15BrowserProfile|SessionExtensionRoundTrip|ContextProfile|ProfileValidators|ContextSemantics|AuthenticationContext|AuthenticationCallVersions|HistoricalNavigate|StepReuse)", "-count=1"},
			Assertions: []string{"immutable older browser and authentication contracts", "UWS 1.9 browser 1.7 scalar semantics", "discriminator-aware schema dispatch", "workflow source and call bindings"},
			RequiredPasses: []string{
				...
				"TestBrowser17ScalarOutputFixtureValidates",
				"TestBrowser17AccessibilityTextRejectsCompositeTypesWithoutChangingBrowser16",
			},
		},
		{
			ID: "udon-browser-consumer", Repository: "udon", Kind: "go_test",
			Args:       []string{"go", "test", "-v", "./internal/sourceloader", "./generator", "./spider", "./pkg/browserdriver", "./pkg/uwsprofile", "./cmd/udon", "-run", "Test(LoadBrowser|BrowserSource|NewRuntimePlanFromUWSFile.*Browser|BrowserRuntime|BrowserAuthentication|BrowsertoolsEnvelope|TypedScalar|SessionIsOpaque|PersistentSubprocess|PersistentProtocols|ValidateAuthenticationRequest|ValidateBrowserAuthenticationCall|ParseExecuteFlagsAcceptsRepeatableBrowserPolicy|BrowserExecutionOptionsRequireExplicitDriver|ConfiguredBrowserExecutionOptions)", "-count=1"},
			Assertions: []string{"trusted runtime keeps source/profile checks", "v2 rejection and v3 browser 1.7 replay", "post-conversion schema and redaction checks", "opaque named sessions", "driver remains explicit"},
			RequiredPasses: []string{
				...
				"TestLoadBrowser17SourcePreservesTypedScalarOutputs",
				"TestNewRuntimePlanFromUWSFileRequiresUWS19AndNamedV3SessionForBrowser17",
				"TestTypedScalarOutputsRetainTypeSchemaAndRedactionValidation",
			},
		},
		{
			ID: "browserdriver-runtime", Repository: "browserdriver", Kind: "npm_test",
			Args:       []string{"npm", "test"},
			Assertions: []string{"closed v2/v3 NDJSON runtime protocol", "browser 1.7 v3-only scalar conversion", "conversion failure non-disclosure", "older extraction compatibility", "offline synthetic runtime tests"},
			RequiredPasses: []string{
				...
				"browser 1.7 converts canonical trimmed accessibility scalars",
				"browser 1.7 rejects empty, noncanonical, non-finite, and out-of-range scalars",
				"browser 1.7 converts a11y text while browser 1.5 and 1.6 extraction remain unchanged",
				"browser 1.7 presence preserves Boolean matching without reading text",
				"browser 1.7 is v3-only and conversion failures disclose no page text",
			},
		},
		{
