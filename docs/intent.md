...
content_trust { ... }                 # optional operator-authored provenance
...
intent should follow it for reviewability: top-level attributes, `workflow`, `content_trust`, inputs, triggers,
...
The optional `content_trust` block records an operator's reviewed integrity
provenance declarations. It is not generated by the LLM, does not contain
content values, and does not grant approval or execution authority.

```hcl
source = "browser-profiles/mail.yaml"

content_trust {
  source_description "browser-profiles/mail.yaml" {
    level = "untrusted"
  }
...
Levels are exactly `unknown`, `trusted`, or `untrusted`. A
`source_description` label is the package-relative path already named by
top-level or step-local `source`; OpenUdon resolves it to the generated UWS
source-description identifier. An `operation` label is the authored leaf step
name and may declare a default, overrides for outputs actually declared by the
generated operation, or both. A `trigger` label must name an authored trigger.
The only generated workflow ID is `main`; its per-input declarations must name
existing `input` blocks.
...
| `content_trust` | UWS 1.9.1 source, operation-output, trigger, and workflow-entry provenance declarations. |
...
- Operator-authored `content_trust` blocks require non-empty declarations,
  exact trust levels, and references that resolve against the same intent;
  generated UWS validation additionally rejects undeclared operation outputs.
