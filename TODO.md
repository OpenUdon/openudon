...
| XRD-007 | `[watch]` | P1 | Repo access / secrets | Private sibling checkout and provider credential availability for local/manual development. | GitHub CI has been removed during active development; deterministic checks and real-LLM eval remain local/manual. | infra / Ramen | Keep `docs/xrd-007-infra-handoff.md` as the local readiness runbook; add automation only after the private dependency layout stabilizes. |
...
3. For XRD-007, keep release and deterministic gates local/manual until private dependency checkout
   and provider-secret handling are stable enough to justify automation.
