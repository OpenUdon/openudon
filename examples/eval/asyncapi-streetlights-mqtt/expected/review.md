...
- `local` pass `/home/peter/Workspace/openudon/examples/eval/asyncapi-streetlights-mqtt/openapi` - OpenAPI directory is absent; using package-local first-class API source documents.

## Inferred Steps And Data Flow

- `dim_streetlight` (http) operation `dimLight`: Publish the Streetlights MQTT dim-light command.
...
```bash
openudon run --example asyncapi-streetlights-mqtt --tier sandbox --approval approvals/asyncapi-streetlights-mqtt.json --dry-run
```

Trusted proof run, only when explicitly approved:

```bash
openudon run --example asyncapi-streetlights-mqtt --tier sandbox --approval approvals/asyncapi-streetlights-mqtt.json
```
