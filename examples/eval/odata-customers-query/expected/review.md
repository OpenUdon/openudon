...
- `query_customers` (http) operation `entitySet.Customers.query`: Read the Customers entity set.
...
```bash
openudon run --example odata-customers-query --tier sandbox --approval approvals/odata-customers-query.json --dry-run
```

Trusted proof run, only when explicitly approved:

```bash
openudon run --example odata-customers-query --tier sandbox --approval approvals/odata-customers-query.json
```
