...
- `local` pass `/home/peter/Workspace/openudon/examples/eval/graphql-starwars-hero/openapi` - OpenAPI directory is absent; using package-local first-class API source documents.

## Inferred Steps And Data Flow

- `get_hero` (http) operation `query.hero`: Read the Star Wars hero field.
...
```bash
openudon run --example graphql-starwars-hero --tier sandbox --approval approvals/graphql-starwars-hero.json --dry-run
```

Trusted proof run, only when explicitly approved:

```bash
openudon run --example graphql-starwars-hero --tier sandbox --approval approvals/graphql-starwars-hero.json
```
