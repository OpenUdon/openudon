...
- `local` pass `/home/peter/Workspace/openudon/examples/eval/grpc-trace-export/openapi` - OpenAPI directory is absent; using package-local first-class API source documents.

## Inferred Steps And Data Flow

- `export_trace` (http) operation `opentelemetry.proto.collector.trace.v1.TraceService/Export`: Submit the OpenTelemetry trace export request.
...
```bash
openudon run --example grpc-trace-export --tier sandbox --approval approvals/grpc-trace-export.json --dry-run
```

Trusted proof run, only when explicitly approved:

```bash
openudon run --example grpc-trace-export --tier sandbox --approval approvals/grpc-trace-export.json
```
