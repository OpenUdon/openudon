...
func runBrowserTransactionEvalCommand(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Fprintf(fs.Output(), "Runs or verifies the canonical, value-free cross-package browser transaction qualification report. Run mode requires clean exact OpenUdon, Browsertools, Browserdriver, Udon, and UWS repositories; independently resolves every origin/main read-only; requires unchanged UWS to be published; accepts the locked implementation commits only as published main or clean local-main descendants while publication is pending; and launches sandboxed Chromium only against embedded loopback fixtures. The report contains only closed gate outcomes, exact commit publication classifications, lifecycle digests, and loopback/sandbox posture; it cannot carry paths, subprocess output, browser content, account identifiers, or credential values.\n\n")
		fs.PrintDefaults()
	}
...
func runTrustedCommand(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "Usage: openudon run --example examples/<name> --tier sandbox|production --approval approvals/<name>.json [--browser-driver /absolute/path] [--browser-driver-arg value] [--browser-registration-attestation /outside/repo/attestation.json --approve-browser-registration OP_ID] [--workdir .openudon-run/<name>] [--dry-run]\n")
